...
The pipeline uses tools (INSERT TOOLS!) for peak-calling and motif analysis, relying on references generated by the [iCount-Mini](https://github.com/ulelab/iCount-Mini) segment script.
...
Pre-filtering is enabled by setting `--skip_gtf_filter` to `false` (or omitting the parameter). This filters the annotation GTF to include only transcripts of interest, which can be provided with `--representative_transcript` or, if not given, automatically selected based on the longest transcript per gene, using the hierarchy (longest CDS, longest total exon length).

iCount-Mini is then executed with both the original and filtered GTFs, generating a segmentation GTF and a flattened regions GTF. Segmentation splits the genome into regions like CDS, UTR3, UTR5, ncRNA, introns, and intergenic.

If segmentation is performed on the filtered annotation, some regions may remain unannotated because the "gene-level" annotation only covers transcripts that remain after filtering. The pipeline identifies these unannotated regions in the iCount genomic segment (regions.gtf) and annotates them according to the corresponding gene using the regions obtained with the unfiltered GTF.
